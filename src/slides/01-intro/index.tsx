  ...
  return (
    <section className="slide stage-bg">
      ...
      <div className="relative z-10 mx-auto grid h-full max-w-7xl items-center gap-8 md:grid-cols-[1.15fr_.85fr]">
        ...
        <Terminal
          lines={['cd university-workshop', 'whoami', 'future-developer']}
          typing="git init future-developers"
        />
      </div>
    </section>
  );
...
function WhySlide() {
  return (
    <SlideShell
      eyebrow="Opening"
      title="لماذا هذه الورشة مهمة؟"
      subtitle="Git ليس أمراً للحفظ، بل طريقة عمل جماعية تجعل المشروع قابلاً للفهم والمراجعة والتوسع."
    >
      <div className="grid h-full items-center gap-5 md:grid-cols-3">
        {[
          ...
          [
            'سوق العمل',
            'Pull Requests و Code Review ليست مفاهيم إضافية؛ إنها جزء من العمل اليومي.',
          ],
...
function ChaosStorySlide() {
  const files = [
    'Student1_final.zip',
    'Student2_final_final.zip',
    'project_REAL_FINAL.zip',
    'backup_before_fix.zip',
    'presentation_last_last.zip',
  ];
  return (
    <SlideShell
      eyebrow="Story"
      title="خمسة طلاب يعملون على مشروع تخرج"
      subtitle="قبل Git، المشكلة ليست في الكود فقط؛ المشكلة في معرفة النسخة الصحيحة ومن غيّر ماذا."
    >
      <div className="grid h-full items-center gap-8 md:grid-cols-2">
        <div className="glass rounded-3xl p-7">
          <div className="mb-5 flex items-center gap-3 text-red-400">
            <AlertTriangle /> قبل Git
          </div>
          <div className="space-y-3 text-left font-mono" dir="ltr">
            {files.map((file, index) => (
              <motion.div
                key={file}
                className="rounded-2xl border border-gh-border bg-black/30 px-5 py-4 text-gh-muted"
        ...
        >
          <div className="mb-6 flex items-center gap-3 text-gh-green">
            <Users /> Git Solution
          </div>
...
function JourneySlide() {
  return (
    <SlideShell
      ...
      title="من لا أعرف لماذا أحتاج Git إلى أستطيع العمل ضمن فريق"
      subtitle="سنبدأ بالمشكلة، ثم النموذج الذهني، ثم الأوامر، ثم التعاون الحقيقي عبر GitHub و GitLab."
    >
      <div className="mt-16 grid gap-4 md:grid-cols-5">
        {['المشكلة', 'Version Control', 'Git عملياً', 'GitHub/GitLab', 'Lab جماعي'].map(
          (item, index) => (
            <motion.div
              key={item}
              ...
              transition={{ delay: index * 0.12 }}
            >
              <div className="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-2xl bg-gl-orange/15 font-mono text-xl text-gl-orange">
                {index + 1}
              </div>
...
export const introSlides: Slide[] = [
  {
    ...
    section: 'الافتتاح',
    title: 'غلاف الورشة',
    keywords: ['hero', 'git init'],
    notes: 'ابدأ بسؤال: كم مرة ضاعت نسخة مشروع أو استبدل أحدكم ملفاً مهماً؟',
    component: HeroSlide,
  },
  {
    id: 'intro-why',
    section: 'الافتتاح',
    title: 'لماذا هذه الورشة مهمة؟',
    keywords: ['importance', 'workflow'],
    notes: 'اربط Git بسوق العمل ومشاريع الجامعة، لا تقدمه كأداة أوامر فقط.',
    component: WhySlide,
  },
  {
    id: 'intro-story',
    section: 'القصة',
    title: 'خمسة طلاب يعملون على مشروع تخرج',
    keywords: ['story', 'zip', 'chaos'],
    notes: 'اعرض أسماء الملفات كمشهد مألوف، ثم قدم Git كحل تنظيمي.',
    component: ChaosStorySlide,
  },
];
